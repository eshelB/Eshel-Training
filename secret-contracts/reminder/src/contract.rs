...
use crate::state::{load, may_load, save, State, Reminder, GENERAL_STATE_KEY};
...
pub fn init<S: Storage, A: Api, Q: Querier>(
    ...
    let general_state = State {
        max_size,
        reminder_count: 0_u64,
    };

    save(&mut deps.storage, GENERAL_STATE_KEY, &general_state)?;
...
fn try_record<S: Storage, A: Api, Q: Querier>(
    ...
    let mut general_state: State = load(&mut deps.storage, GENERAL_STATE_KEY)?;

    if reminder.len() > general_state.max_size.into() {
        ...
        general_state.reminder_count += 1;
        save(&mut deps.storage, GENERAL_STATE_KEY, &general_state)?;
...
fn query_stats<S: Storage, A: Api, Q: Querier>(deps: &Extern<S, A, Q>) -> StdResult<Binary> {
    // retrieve the general_state state from storage
    let general_state: State = load(&deps.storage, GENERAL_STATE_KEY)?;
    to_binary(&QueryAnswer::Stats{ reminder_count: general_state.reminder_count })
}
